import { default as Meld, Pair, Chow, ExposedPung, ConcealedPung, Knitted, FullyKnitted, ThirteenOrphans } from "../meld"
    ...
    if (this.hand.isNormalHand()) {
      ...
      for (let result of expand(tiles, this.hand.size + 1, 0)) {
        ...
        result.sort((x, y) => {
          ...
          } else if ([Meld.PAIR, Meld.EXPOSED_PUNG, Meld.CONCEALED_PUNG, Meld.EXPOSED_KONG, Meld.CONCEALED_KONG].includes(x.type)) {
            return x.tile - y.tile
          } else if (x.type === Meld.CHOW) {
