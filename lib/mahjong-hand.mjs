...
export default class MahjongHand extends Hand {
  ...
  * getAllCombinations() {
    let waitTile = this.waitTile
    let isSelfDrawn = this.isSelfDrawn
    function* expand(tiles, size, min = 0) {
      ...
      for (let tile = 27; tile < 34; ++tile) {
        if (tiles[tile] === 2) {
          ...
          tiles[tile] = 0
          size -= 3
          let Pung = waitTile === tile && !isSelfDrawn ? ExposedPung : ConcealedPung
  ...
  getScore() {
    ...
    for (let melds of this.getAllCombinations()) {
      if (!firstCombination) {
        firstCombination = melds
      }
      let {score, fans} = new Scoring(this, melds, waitTiles.length === 1).getScore()
      if (score > best.score) {
        best = {score, fans, melds}
      }
    }
    if (best.score === 0) {
      best.score = 8
      best.fans = [{name: '无番和', melds: best.melds}]
    }
    if (best.score >= 8) {
      best.score += this.flowers
      for (let i = 0; i < this.flowers; ++i) {
        best.fans.push({name: '花牌', melds: []})
      }
    }
    return best
  }
