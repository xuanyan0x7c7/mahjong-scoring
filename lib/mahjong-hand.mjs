...
import { QuadrupleChow, FourPureShiftedPungs } from './scoring/48'
import { FourShiftedChows, ThreeKongs, AllTerminalsAndHonors } from './scoring/32'
import {
...
export default class MahjongHand extends Hand {
  ...
  getScore({isSelfDrawn = false, isFourthTile = false, isKong = false, isLastTile = false} = {}) {
    ...
    for (let melds of scoring.getAllCombinations()) {
      ...
      for (let Class of procedureList) {
        new Class(
          this, melds, fans, {omittes, combinations},
          {isSelfDrawn, isFourthTile, isKong, isLastTile, isSingleWait: waitTiles.length === 1}
        ).process()
      }
      let score = fans.map(fan => fanMap[fan]).reduce((x, y) => x + y, 0)
      if (bestScore < score) {
        bestScore = score
        bestFans = fans
      }
